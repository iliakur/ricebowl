                ...
                ((agenda
                 ...
                 (alltodo
                  ...
                  ((org-agenda-files (list (expand-file-name "work/tasks.org" org-directory)))
                   (org-super-agenda-groups
                    '((:name "In progress" :tag "wip")
                      (:name "Waiting/Blocked" :todo "WAITING")
                      (:auto-parent t)
                      (:auto-tags t)))))))
               ("p" . "Personal")
               ("pp" "Personal Daily"
                  ...
                  ((org-agenda-dim-blocked-tasks t)
                   (org-super-agenda-groups
                    '((:name "Habits" :habit t :order 2)
                      (:name "Schedule" :scheduled t :deadline t :order 1)
                      (:name none :discard (:anything t))))))
                 (alltodo
                  ""
   ...
   #+begin_src emacs-lisp
     ...
     (setq org-refile-targets
           '(("~/Documents/org/gtd.org" :maxlevel . 2)
             ...
             ("~/Documents/org/work/rtr.org" :maxlevel . 2)))
   #+end_src

*** TODO move refiling targets here
** Exporting
*** Disable formalities
#+begin_src emacs-lisp
